...
| `POST` | `/api/auth/prekeys` | Yes | 20 / 1 min per user |
| `POST` | `/api/auth/keys` | Yes | 20 / 1 min per user |
| `DELETE` | `/api/auth/user/:userId` | Yes | 20 / 1 min per user |
| `POST` | `/api/auth/session` | Yes | 20 / 5 min per user |
| `POST` | `/api/auth/block` | Yes | 20 / 1 min per user |
| `POST` | `/api/auth/unblock` | Yes | 20 / 1 min per user |
| `GET` | `/api/auth/blocked` | Yes | 20 / 1 min per user |
| `POST` | `/api/messages/send` | Yes | 120 / 1 min per user |
| `GET` | `/api/messages/pending/:userId` | Yes | 240 / 1 min per user |
| `DELETE` | `/api/messages/:messageId` | Yes | 240 / 1 min per user |
| `POST` | `/api/messages/acknowledge` | Yes | 240 / 1 min per user |
...
| `GET` | `/api/health` | No | 60 / 1 min per IP |
| `GET` | `/api/metrics` | Prod: `X-Metrics-Token` | 60 / 1 min per IP |
...
```typescript
{
  ...
  recipientId: string         // UUID of the recipient (not username — prevents enumeration)
...
Server metrics. **Enabled in production**, gated by an `X-Metrics-Token` header that must match `METRICS_SECRET`; the endpoint returns `403` when the token is absent or wrong, and also `403` when `METRICS_SECRET` is unset (fails closed). In non-production the token is not required. Rate-limited at 60 / 1 min per IP.
...
```typescript
{
  token: string       // Base64url-encoded token, expires in 24 hours
...
```typescript
{
  ...
  isPublic?: boolean      // Optional; marks the file as public (any authenticated user may download).
                          // Used for non-E2EE profile avatars and group attachments (encrypted blobs).
}
```
...
Access control (**deny by default**): only the uploader, the designated recipient (if `recipientId` was set at upload), or files explicitly marked `isPublic: true` (e.g. profile avatars — intentionally non-E2EE — and group attachments, whose contents stay protected by the per-member E2E file key) can be downloaded. A file with no recipient and `isPublic=false` is downloadable only by its uploader.
...
The `encryptedPayload` field in message endpoints is a JSON string with a maximum size of **64 KB**. Only the **v2 (Double Ratchet)** envelope is accepted: the server rejects legacy **v1 (nacl-box)** payloads with `400`, and the client fails closed (never decrypts or displays them).

### v1: NaCl Box (Removed)

The legacy `v:1, alg:"nacl-box"` envelope is **no longer supported**. The server rejects it with `400 Invalid encrypted payload` and the client never decrypts or displays it. Every message MUST use the v2 Double Ratchet envelope below.
...
Optional on the v2 envelope. If present, must be a finite number between `0` and `604800` (7 days in seconds). Interpretation is client-side -- the server does not enforce message deletion based on this value.

### Validation Rules

- The entire JSON string must not exceed 64 KB (UTF-8 byte length).
- `v` must be `2` with `alg: "lume-ratchet"`. Legacy `v:1` (nacl-box) is rejected.
- All Base64 keys must decode to exactly 32 bytes.
- Nonces must decode to 24 bytes (`nacl.secretbox.nonceLength`).
- Ciphertext must be non-empty and not exceed 64 KB decoded.
- Payloads that do not match the v2 format are rejected (including legacy v1 nacl-box).
