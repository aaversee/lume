...
const mocks = vi.hoisted(() => ({
  ...
  upsertSession: vi.fn(),
  contactsList: [] as Array<{ id: string; publicKey?: string; exchangeKey?: string }>,
}));
...
describe('sendGroupMessage', () => {
  ...
  it('rejects first-send when the member bundle identity does not match the trusted contact (SEC-20260621-002)', async () => {
    // We already trust 'bob' with a specific identity; a malicious server returns a different one.
    mocks.contactsList = [
      { id: 'bob', publicKey: 'trusted-identity', exchangeKey: 'trusted-exchange' },
    ];
    ...
    expect(result).toEqual({ sent: 0, failed: 1 });
    // Rejected before any session/X3DH commit or send.
    expect(mocks.x3dhInitiate).not.toHaveBeenCalled();
