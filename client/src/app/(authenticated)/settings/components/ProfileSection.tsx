...
export default function ProfileSection() {
  ...
  const [, setAvatarFileId] = useState<string | null>(null);
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!ACCEPTED_TYPES.includes(file.type)) {
      setError(t("settings.profile.errorImageType"));
      return;
    }
    if (file.size > MAX_FILE_SIZE) {
      setError(t("settings.profile.errorImageSize"));
      return;
    }
    ...
    try {
      ...
      // Upload file
      // Avatars are an explicit public (non-E2EE) profile asset so contacts can
      // fetch them; they are uploaded as public and never gated by recipient ACL. SEC-20260621-005.
      const uploadRes = await filesApi.upload(base64, file.type, { isPublic: true });
      if (uploadRes.error || !uploadRes.data) {
        setError(uploadRes.error ?? t("settings.profile.errorUpload"));
        return;
      }
      ...
      setError(t("settings.profile.errorAvatar"));
    } finally {
      setUploading(false);
    }
  }, [userId]);
  ...
  const saveDisplayName = useCallback(async () => {
    ...
    try {
      ...
      setError(t("settings.profile.errorDisplayName"));
    } finally {
      setSaving(false);
    }
  }, [userId]);
  ...
  if (!loaded) {
    return (
      <section>
        <SectionHeading>{t("settings.profile.title")}</SectionHeading>
      ...
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          <div>
            ...
            />
            {saving ? (
              <p className="text-caption text-[var(--text-muted)] mt-1 animate-pulse">
                {t("settings.profile.saving")}
              </p>
            ) : saved ? (
              <p className="text-caption text-[var(--accent)] mt-1">
