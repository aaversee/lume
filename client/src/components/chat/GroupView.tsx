...
export default function GroupView({ group }: GroupViewProps) {
  ...
  const typingLabel = useMemo(() => {
    ...
    if (typingNames.length === 1) return t("group.typingOne", { name: typingNames[0] ?? "" });
    if (typingNames.length === 2)
      return t("group.typingTwo", { first: typingNames[0] ?? "", second: typingNames[1] ?? "" });
    return t("group.typingMany");
  }, [typingNames]);
  ...
  const handleAttach = useCallback((file: File) => {
    if (file.size > MAX_FILE_SIZE) {
      alert(t("group.fileTooLarge", { size: formatFileSize(MAX_FILE_SIZE) }));
      return;
    }
  ...
  const handleSend = async () => {
    ...
    if (pendingAttachment) {
      try {
        ...
        // Group attachment fans out to many members, so the encrypted blob is
        // uploaded as public-download; its content stays protected by the E2E file
        // key carried in each per-member payload. SEC-20260621-006.
        ...
        // Metadata only in message/Zustand state; the decrypt key/nonce travel
        // E2E in each member's payload and a local copy lives in the vault. SEC-20260621-004.
        attachmentMeta = {
    ...
    <div className="h-full min-h-0 overflow-hidden flex flex-col">
      ...
      <div className="px-5 py-4 border-b border-[var(--border)]/70 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          <button
            ...
            aria-label={t("group.backToGroups")}
          >
        ...
        <button
          ...
          aria-label={mode === "chat" ? t("group.info") : t("group.backToChat")}
          title={mode === "chat" ? t("group.info") : t("group.backToChat")}
        >
          {mode === "chat" ? (
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
      ...
      {mode === "chat" ? (
        <>
              ...
              <>
                {groupMessages.map((m) => {
                  ...
                  return (
                    <div key={m.id}>
                      {!isMine && (
                        <p className="px-1 mb-0.5 text-caption text-[var(--text-muted)]">
                          @{senderName}
                        </p>
                      )}
                      <MessageBubbleMemo
          ...
          {typingLabel && (
            <div className="px-5 pb-1 text-caption text-[var(--text-muted)] italic">
              {typingLabel}
            </div>
          )}
            ...
            {replyingTo && (
              <div className="mb-3 flex items-start gap-3 px-4 py-2.5 rounded-[var(--radius-md)] bg-[var(--surface-alt)] border border-[var(--border)]">
                <div className="flex-1 min-w-0 pl-3 border-l-2 border-[var(--accent)]">
                  <p className="text-caption font-semibold text-[var(--accent)] mb-0.5">
            ...
            <div className="flex items-end gap-3">
              <div className="flex-1">
                <textarea
                  ...
                  placeholder={hasMembersToSend ? t("chat.messagePlaceholderPlain") : t("group.addMembersToChat")}
                  rows={1}
                  maxLength={10000}
                  disabled={!hasMembersToSend}
                  aria-label={t("group.messageInputAria")}
                  className="w-full px-2 py-2.5 bg-transparent text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none resize-none text-base leading-snug disabled:opacity-60"
              ...
              <button
                ...
                disabled={!hasMembersToSend}
                className="w-10 h-10 rounded-full text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-alt)] transition-colors disabled:opacity-60 disabled:cursor-not-allowed inline-flex items-center justify-center flex-shrink-0"
              ...
              <button
                ...
                disabled={!hasMembersToSend}
                className={`w-10 h-10 rounded-full transition-colors inline-flex items-center justify-center flex-shrink-0 disabled:opacity-60 disabled:cursor-not-allowed ${
                  selfDestructTime
                    ? "bg-[var(--surface-alt)] text-[var(--text-primary)]"
                    : "text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-alt)]"
                }`}
                aria-label={t("chat.selfDestructTimer")}
                title={
                  selfDestructTime
                    ? `Auto-delete: ${formatTimerLabel(selfDestructTime)}`
                    : t("chat.selfDestructTimer")
                }
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              ...
              <button
                ...
                className="w-11 h-11 rounded-full bg-[var(--accent)] text-[var(--accent-contrast)] hover:bg-[var(--accent-hover)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed inline-flex items-center justify-center flex-shrink-0"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
          ...
          <div className="mb-8">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-body font-semibold text-[var(--text-secondary)]">
            ...
            <div>
              {group.members.map((member) => {
                ...
                return (
                  <div
                    key={member.user_id}
                    className="py-3 flex items-center gap-3 border-b border-[var(--border)]/55 last:border-b-0"
                  >
                    ...
                    {canRemove && (
                      <button
                        ...
                        aria-label={t("group.removeMember", { username: member.username })}
                        title={t("group.removeMember", { username: member.username })}
                      >
                        {isRemoving ? (
          ...
          <div className="mb-8">
            <h3 className="text-body font-semibold text-[var(--text-secondary)] mb-1">
              {t("group.infoHeading")}
            </h3>
            <div className="flex items-center justify-between py-3 border-b border-[var(--border)]/55">
              <span className="text-body text-[var(--text-secondary)]">{t("group.created")}</span>
