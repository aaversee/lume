...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { data, mimeHint, recipientId, isPublic } = req.body as {
        data: string
        mimeHint?: string
        recipientId?: string
        isPublic?: boolean
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Access control (deny by default): only the uploader, the designated
      // recipient, or an explicitly public file may be downloaded. SEC-20260621-006.
      if (
        !file.is_public &&
