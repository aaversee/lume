...
// Pragmas
// auto_vacuum must be set BEFORE journal_mode = WAL: SQLite silently ignores an
// auto_vacuum change once the connection is in WAL mode, which left it at NONE
// and made every PRAGMA incremental_vacuum a no-op. SEC-20260721-013.
...
// One-time conversion for databases created before the pragma-order fix above.
// On those the mode is still NONE (0), and it cannot be changed while in WAL
// mode — so drop to a rollback journal, set INCREMENTAL, VACUUM to rewrite the
// file under the new mode, then return to WAL. Guarded on the mode, so it runs
// once: after the VACUUM the mode is 2 and this is skipped on every later boot.
// A fresh DB already reads 2 here (set before WAL), so it is skipped too.
// The VACUUM preserves all data but rewrites the whole file and briefly locks
// it, which is why it is one-time and guarded. SEC-20260721-013.
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_messages_recipient_sender ON pending_messages(recipient_id, sender_id);
...
const countPendingFromSender = db.prepare(`
  SELECT COUNT(*) as count FROM pending_messages WHERE recipient_id = ? AND sender_id = ?
`)
 ...
 * returned to the filesystem only when this runs. Call it after a bulk delete
 * that actually removed rows, so the file tracks live size rather than growing
 * monotonically to its high-water mark. SEC-20260721-013.
 */
function reclaimFreePages(): void {
...
const countBlocksByBlockerStmt = db.prepare(`
  SELECT COUNT(*) AS c FROM blocked_users WHERE blocker_id = ?
`)
...
const insertFile = db.prepare(`
  INSERT INTO files (id, uploader_id, size, mime_hint, expires_at, recipient_id, is_public)
  VALUES (?, ?, ?, ?, ?, ?, ?)
`)
...
const countGroupsByCreatorStmt = db.prepare(`
  SELECT COUNT(*) AS c FROM groups WHERE creator_id = ?
`)
...
export const database = {
  ...
  getPendingMessageCountFromSender(recipientId: string, senderId: string): number {
    const result = countPendingFromSender.get(recipientId, senderId) as { count: number }
    return result.count
  },

  /**
   * Live page stats. Exposed so freelist growth is observable — with
   * `auto_vacuum = INCREMENTAL` a rising freelist means bulk deletes are not
   * reclaiming, which is the failure SEC-20260721-013 guards against.
   */
  getStoragePageStats(): { pageCount: number; freelistPageCount: number; autoVacuum: number } {
    return {
      pageCount: db.pragma('page_count', { simple: true }) as number,
      freelistPageCount: db.pragma('freelist_count', { simple: true }) as number,
      // auto_vacuum mode (2 = INCREMENTAL). If this is 0 the incremental_vacuum
      // calls are no-ops — the exact regression SEC-20260721-013 turned out to be.
  ...
  deleteAllMessages(recipientId: string): void {
    const changes = deleteUserMessages.run(recipientId).changes
    if (changes > 0) reclaimFreePages()
  },
  ...
  purgeStaleMessages(maxAgeSec: number): number {
    ...
    if (result.changes > 0) reclaimFreePages()
  ...
  cleanupRequestSignatures(cutoff: number): number {
    const changes = cleanupOldRequestSignatures.run(cutoff).changes
    if (changes > 0) reclaimFreePages()
    return changes
  },
  ...
  countBlockedByUser(blockerId: string): number {
    return (countBlocksByBlockerStmt.get(blockerId) as { c: number }).c
  },
  ...
  purgeExpiredFiles(nowSec: number): string[] {
    ...
    if (rows.length > 0) reclaimFreePages()
  ...
  deleteExpiredInviteTokens(nowSec: number): number {
    const changes = deleteExpiredInviteTokens.run(nowSec).changes
    if (changes > 0) reclaimFreePages()
    return changes
  },
  ...
  countGroupsByCreator(creatorId: string): number {
    return (countGroupsByCreatorStmt.get(creatorId) as { c: number }).c
  },
