...
function makePayload() {
  return JSON.stringify({
    ...
    alg: 'lume-ratchet',
    header: {
      publicKey: encodeBase64(nacl.randomBytes(32)),
...
describe('E2E chat flow', () => {
  ...
  it('send → pending → single ack: full round trip', async () => {
    ...
    const sendBody = { senderId: aliceId, recipientId: bobId, encryptedPayload: payload };
  ...
  it('batch acknowledge removes multiple messages', async () => {
    ...
    // Send two messages
    const body1 = { senderId: aliceId, recipientId: bobId, encryptedPayload: makePayload() };
    ...
    const body2 = { senderId: aliceId, recipientId: bobId, encryptedPayload: makePayload() };
  ...
  it('blocked sender message is silently accepted but not stored', async () => {
    ...
    const sendBody = { senderId: aliceId, recipientId: bobId, encryptedPayload: makePayload() };
  ...
  it('send to non-existent recipient returns 404', async () => {
    ...
    const body = { senderId: aliceId, recipientId: '00000000-0000-4000-8000-0000000000aa', encryptedPayload: makePayload() };
  ...
  it('empty encrypted payload returns 400', async () => {
    ...
    const body = { senderId: aliceId, recipientId: bobId, encryptedPayload: '' };
    const h = signHeaders('POST', '/messages/send', body, alice.idKey);
    const res = await request(app).post('/api/messages/send').set(h).send(body);
  ...
  it('invalid senderId format returns 400', async () => {
    ...
    const body = { senderId: 'bad-id', recipientId: '00000000-0000-4000-8000-0000000000bb', encryptedPayload: makePayload() };
    const h = signHeaders('POST', '/messages/send', body, alice.idKey);
    const res = await request(app).post('/api/messages/send').set(h).send(body);
    expect(res.status).toBe(400);
  });
  ...
  it('non-existent senderId returns 401', async () => {
    ...
    const body = { senderId: fakeId, recipientId: '00000000-0000-4000-8000-0000000000bb', encryptedPayload: makePayload() };
    const h = signHeaders('POST', '/messages/send', body, alice.idKey);
    const res = await request(app).post('/api/messages/send').set(h).send(body);
  ...
  it('identity key mismatch returns 403', async () => {
    ...
    const body = { senderId: aliceId, recipientId: bobId, encryptedPayload: makePayload() };
  ...
  it('missing auth headers returns 401', async () => {
    const res = await request(app).post('/api/messages/send').send({
      senderId: 'x', recipientId: 'y', encryptedPayload: 'z',
    });
    expect(res.status).toBe(401);
  });
});
